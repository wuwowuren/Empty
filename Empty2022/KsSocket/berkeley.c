...
)
{
	...
	PADDRINFOEXW Result = ExAllocatePoolWithTag(NonPagedPool, sizeof(ADDRINFOEXW), MEMORY_TAG);
...
)
{
	...
	PADDRINFOA Result = ExAllocatePoolWithTag(NonPagedPool, sizeof(ADDRINFOA), MEMORY_TAG);
...
int socket_listenV2(PWSK_SERVER_INFO_BASE Socket, int domain, int type, int protocol)
{
	NTSTATUS Status;
	//
	// WskSocket() returns STATUS_PROTOCOL_UNREACHABLE (0xC000023E)
	// when Protocol == 0, so coerce this value to IPPROTO_TCP here.
	//
	int sockfd = TO_SOCKETFD(Socket->KsIndex++);

	Status = KsCreateListenSocketV2(
		&Socket->kSocket[sockfd],
		(ADDRESS_FAMILY)domain,
		(USHORT)type,
		protocol ? (ULONG)protocol : IPPROTO_TCP
	);
...
int bindV2(PWSK_SERVER_INFO_BASE Socket, int sockfd, const struct sockaddr* addr, socklen_t addrlen)
{
	UNREFERENCED_PARAMETER(addrlen);
	...
	Status = KsBind(&Socket->kSocket[sockfd], (PSOCKADDR)addr);
...
int closesocketV2(PWSK_SERVER_INFO_BASE Socket, int sockfd)
{
	NTSTATUS Status;

	Status = KsCloseSocket(&Socket->kSocket[sockfd]);

	//Socket->kSocket[sockfd] = 0;
...
//int listenV2(PWSK_SERVER_INFO_BASE Socket, int sockfd, int backlog)
//{
//	return 0;
//}
