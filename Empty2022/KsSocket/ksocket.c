...
WSK_SERVER_INFO_BASE * pBaseWsk = 0;


#define wWskDispatch  (pBaseWsk->WskDispatch)
#define wWskProvider (pBaseWsk->WskProvider)
#define wWskRegistration (pBaseWsk->WskRegistration)
//#define pBaseWsk->
...
)
{
	...
	WSK_CLIENT_DISPATCH  NowWskDispatch = { MAKE_WSK_VERSION(1,0), 0, NULL };
	pBaseWsk->WskDispatch = NowWskDispatch;
	//
	...
	WskClient.Dispatch = &wWskDispatch;

	Status = WskRegister(&WskClient, &wWskRegistration);
...
)
{
	...
	WskReleaseProviderNPI(&wWskRegistration);

	//
	// Deregister as a WSK client.
	//

	WskDeregister(&wWskRegistration);
}
...
)
{
	...
	//pBaseWsk->WskProvider

	Status = wWskProvider.Dispatch->WskGetAddressInfo(
		wWskProvider.Client,         // Client
		NodeName,                   // NodeName
...
{
	wWskProvider.Dispatch->WskFreeAddressInfo(
		wWskProvider.Client,         // Client
		AddrInfo                    // AddrInfo
	);
}
...
)
{
	...
	PKSOCKET NewSocket = ExAllocatePoolWithTag(NonPagedPool, sizeof(KSOCKET), MEMORY_TAG);
	...
	Status = wWskProvider.Dispatch->WskSocket(
		wWskProvider.Client,         // Client
...
NTSTATUS KsCreateSocketV2(PKSOCKET Socket, ADDRESS_FAMILY AddressFamily, USHORT SocketType, ULONG Protocol, ULONG Flags)
{
	NTSTATUS Status;
	...
	Status = KspAsyncContextAllocate(&Socket->AsyncContext);
	...
	if (NT_SUCCESS(Status))
	{
		Socket->WskSocket = (PWSK_SOCKET)Socket->AsyncContext.Irp->IoStatus.Information;
		Socket->WskDispatch = (PVOID)Socket->WskSocket->Dispatch;
		//*Socket = NewSocket;
	}
...
NTSTATUS KsCreateListenSocketV2(PKSOCKET Socket, ADDRESS_FAMILY AddressFamily, USHORT SocketType, ULONG Protocol)
{
	return KsCreateSocketV2(Socket, AddressFamily, SocketType, Protocol, WSK_FLAG_LISTEN_SOCKET);
}
...
)
{
	...
	if (NT_SUCCESS(Status))
	{
		PKSOCKET KNewSocket = ExAllocatePoolWithTag(NonPagedPool, sizeof(KSOCKET), MEMORY_TAG);
